...
function sendMessage() {
    ...
    var checkbox = document.getElementById('checkbox');
        let checked = 1
        // Check if the checkbox is active (checked)
        if (checkbox.checked) {
    ...
    sendUserInputToServer(userInput,checked);
...
function sendUserInputToServer(userInput,checked) {
    $.ajax({
        type: "POST",
        url: "/chat_new",
        data: {"user_input": userInput,"rag":checked},
        success: function(response) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            newDivElement.innerHTML = `
                <div id="chat-title">Custom app</div>
...
function next1(new_name, session_name) {
    ...
    var elementToRemove1 = document.getElementById("chat-container");
    if (elementToRemove1) {
        elementToRemove1.remove();
    } else {
        console.error("Element not found.");
    ...
    var finaldiv = document.getElementById("right");

    var session_con = document.createElement("div");
    session_con.setAttribute("id","chat-container")
    ...
    session_con.appendChild(appendable);
    finaldiv.appendChild(session_con);
