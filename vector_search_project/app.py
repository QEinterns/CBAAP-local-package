...
from app_python_srcs.query_enhancer import query_enhancer
...
with open('execution_time.txt', 'a') as file:
    ...
    def imagine(movie_name,query,text_to_image):
        main.driver_function(movie_name, query,text_to_image,embed_model,script_dir)
    ...
    def chat():
        ...
        while(g):
            ...
            bot_response = response['message']['content']
            # bot_response = query_enhancer(context,user_input,script_dir)
            if(bot_response!=""):
                break
        # print(bot)
